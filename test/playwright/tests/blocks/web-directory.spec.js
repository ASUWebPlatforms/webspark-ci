...
test.describe(`${BLOCK} block tests`, { tag: ['@webspark', '@desktop', '@blocks'] }, () => {
  ...
  test('create (people)', async () => {
    pageUrl = await drupal.createPage(page, `${BLOCK} (people)`);
    await page.goto(pageUrl);
    await page.getByRole('link', { name: 'Layout' }).click();
    await drupal.addBlock(page, BLOCK);

    //--- Begin custom test steps
    await page.getByLabel('Required Component type').selectOption({ label: 'People' });
  ...
  test('verify (people)', async () => {
    ...
    listView = page.getByRole('button', { name: 'List view' });
    next = page.getByRole('button', { name: 'Next Page' });
    prev = page.getByRole('button', { name: 'Previous Page' });
  ...
  test('create (people in departments)', async () => {
    pageUrl = await drupal.createPage(page, `${BLOCK} (people)`);
    await page.goto(pageUrl);
    await page.getByRole('link', { name: 'Layout' }).click();
    await drupal.addBlock(page, BLOCK);
  ...
  test('verify (people in departments)', async () => {
    await page.goto(pageUrl);
  });

  test('create (departments)', async () => {
    await page.goto(pageUrl);
    await page.getByRole('link', { name: 'Layout' }).click();
    await drupal.addBlock(page, BLOCK);
  ...
  test('verify (departments)', async () => {
    await page.goto(pageUrl);
  });

  test('create (faculty by rank)', async () => {
    await page.goto(pageUrl);
    await page.getByRole('link', { name: 'Layout' }).click();
    await drupal.addBlock(page, BLOCK);
  ...
  test('verify (faculty by rank)', async () => {
    await page.goto(pageUrl);
  });
});
